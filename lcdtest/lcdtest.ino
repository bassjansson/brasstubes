...
void lcd_write_u8(const uint8_t data) { lcd_data(&data, 1); }
...
void fillScreen(uint32_t color) { fillRect(0, 0, _width, _height, color); }
...
void setup() {
    ...
    adc1_config_channel_atten((adc1_channel_t)ADC_CHANNEL_8, ADC_ATTEN_DB_11);

    gpio_pad_select_gpio((gpio_num_t)POWER_PIN);
    /* Set the GPIO as a push/pull output */
    gpio_set_direction((gpio_num_t)POWER_PIN, GPIO_MODE_OUTPUT);

    /* Blink off (output low) */
    printf("Turning on the peripherals power\n");
    gpio_set_level((gpio_num_t)POWER_PIN, 1);
